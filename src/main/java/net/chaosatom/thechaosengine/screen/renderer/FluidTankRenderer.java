    ...
    public List<Component> getTooltip(FluidStack fluidStack, TooltipFlag tooltipFlag) {
        ...
        try {
            if (fluidType.isSame(Fluids.EMPTY)) {
                ...
                tooltip.add(Component.translatable("tooltip.thechaosengine.liquid.amount.with.capacity", 0, nf.format(capacity)).withStyle(ChatFormatting.GRAY));
                return tooltip;
            }
            ...
            if (tooltipMode == TooltipMode.SHOW_AMOUNT_AND_CAPACITY) {
                MutableComponent amountString = Component.translatable("tooltip.thechaosengine.liquid.amount.with.capacity", nf.format(milliBuckets), nf.format(capacity));
                tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
            } else if (tooltipMode == TooltipMode.SHOW_AMOUNT) {
                MutableComponent amountString = Component.translatable("tooltip.thechaosengine.liquid.amount", nf.format(milliBuckets));
                tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
            }
        } catch (RuntimeException e) {
            TheChaosEngine.LOGGER.error("Failed to get tooltip for fluid: " + e);
        }
